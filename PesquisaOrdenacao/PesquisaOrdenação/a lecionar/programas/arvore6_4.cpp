...
#include <string.h>
/*
falta remover no com 2 filhos. Procurar no mais a esquerda (sem filhos)
do no diretamente a direita e substituir (eliminar o nó mais a esquerda 
*/
	typedef struct no {
   		struct no *esq;
   		int valor;
...
void removerdoisfilhos(tipo_no **tree);
...
//	printf("Dois filhos %d\n",(*tree)->valor);
	...
	if (prox->esq == NULL)
	{
		(*tree)->valor = prox->valor;
		ant->dir = prox->dir;		//funciona tanto apontando vazio quanto outro no
	}
	else
	{
		while (prox->esq != NULL)
